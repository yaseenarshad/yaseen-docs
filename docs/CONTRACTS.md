...
8. **Empty list items** (`src/editor/listItemRoundTrip.ts`): an empty bullet/ordered item is written as a bare marker (`*` / `1.`), never `* <br />` — `<br />` opens a CommonMark HTML block that swallows the item's nested children on reload. Parsing gives an item whose nested list starts on a later line an empty leading paragraph (`* ` + children, Obsidian's empty parent); `* 1) text` on one line stays a list-first item. Legacy `* <br />` lines are rewritten to bare markers before parsing. Empty TASK items keep `* [ ] <br />` (remark drops the checkbox from `* [ ]`).
