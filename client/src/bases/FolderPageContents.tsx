 ...
 * `folder_page: true` renders its members BELOW its own body — today's fully interactive bases
 ...
 *  - THE ADAPTER (🔒 D3): BaseView stays ONE component. This host builds a def in memory from
 *    `folderPageSettings(record).views`, hands it over as a `ParsedBase`, and turns every def
 ...
 *
 * Which view is active is SESSION state (BaseView's own `active`), never written to the card —
...
import { BaseView, type FolderPageMode } from './BaseView'
 ...
 */
function folderPageBase(views: readonly BaseViewDef[]): ParsedBase {
  try {
    return parseBase(stringify({ views }))
  } catch {
    ...
    return parseBase(stringify({ views: DEFAULT_VIEWS.map((view) => ({ ...view })) }))
  }
}
...
export function FolderPageContents({ path, root, source, onOpenFile, onOpenFileBackground }: FolderPageContentsProps) {
  ...
  const [parsed, setParsed] = useState<ParsedBase | null>(() => (settings === null ? null : folderPageBase(settings.views)))
  ...
  useEffect(() => {
    ...
    seen.current = stamp
    setParsed(settings === null ? null : folderPageBase(settings.views))
  }, [stamp, settings])
  ...
  const onChange = (next: ParsedBase): void => {
    setParsed(next)
    setError(null)
