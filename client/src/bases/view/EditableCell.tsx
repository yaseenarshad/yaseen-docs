...
export function EditableCell({ path, propKey, raw, value, editor, basenames }: EditableCellProps) {
  ...
  const failure = error !== null && (
    <span className="base-table__chip base-table__chip--error base-cell-edit__error" role="alert" title={error}>
      Save failed
    </span>
  )
  ...
  if (editor === 'checkbox') {
    return (
      <span ref={wrapRef} className="base-cell-edit">
        <input
          type="checkbox"
          data-edit=""
    ...
    <span ref={wrapRef} className="base-cell-edit">
      {!editing ? (
        <>
          <button
            type="button"
            className="base-cell-edit__display"
            data-edit=""
            onClick={() => {
              setError(null)
              setEditing(true)
        ...
        <TextField
          className="base-input base-cell-edit__input"
...
function ChipsEditor({ initial, label, basenames, onCommit, onDone }: ChipsEditorProps) {
  ...
  return (
    <span
      className="base-cell-edit__chips"
      onBlur={(e) => {
    ...
    >
      {items.map((item, i) => (
        <span key={`${item}:${i}`} className="base-table__chip">
          {item}
          <button
            type="button"
...
function LinkEditor({ initial, basenames, label, onCommit, onDone }: LinkEditorProps) {
  ...
  return (
    <span className="base-cell-edit__link">
      <input
        className="base-input base-cell-edit__input"
        autoFocus
        aria-label={label}
