/**
 * Cards view (4E, GRO-2139): BaseView mounted with react-dom in jsdom; `type: cards` renders a
...
import { type ParsedBase, parseBase } from '../baseFile'
...
const cards = (el: ParentNode): HTMLElement[] => [...el.querySelectorAll<HTMLElement>('.base-card')]
const cardOf = (el: ParentNode, title: string): HTMLElement => {
  const hit = cards(el).find((c) => c.querySelector('.base-card__title')?.textContent === title)
  if (hit === undefined) throw new Error(`no card titled ${title}`)
  return hit
}
const titles = (el: ParentNode): string[] => [...el.querySelectorAll('.base-card__title')].map((b) => b.textContent ?? '')
const sections = (el: ParentNode): HTMLElement[] => [...el.querySelectorAll<HTMLElement>('.base-cards__group')]
const headerTexts = (el: ParentNode): string[] => sections(el).map((s) => q(s, '.base-group__value').textContent ?? '')
const gridVar = (el: ParentNode, name: string): string => q<HTMLElement>(el, '.base-cards').style.getPropertyValue(name)
...
describe('grid', () => {
  it('renders one flat grid of cards — file name title over typed label/value rows; the title opens the note', () => {
    ...
    expect(el.querySelector('.base-cards__grid')).not.toBeNull()
    expect(el.querySelector('.base-cards__group')).toBeNull()
    expect(cards(el)).toHaveLength(8)
    const card = cardOf(el, 'Agentic Agency.md')
    expect([...card.querySelectorAll('.base-card__prop-name')].map((n) => n.textContent)).toEqual(['priority', 'tags'])
    expect([...card.querySelectorAll('.base-card__prop-value')][0].textContent).toBe('2')
    ...
    expect([...bare.querySelectorAll('.base-card__prop-value')][0].textContent).toBe('')
    click(q(card, '.base-card__title'))
...
describe('cardSize', () => {
  it('numeric px is honored; the board presets map through the shared widths; default medium 280', () => {
    ...
    expect(gridVar(el, '--base-card-w')).toBe('280px')
    unmount()
    const numeric = mount(CARDS_BASE.replace('name: C', 'name: C\n    cardSize: 200'))
    expect(gridVar(numeric.el, '--base-card-w')).toBe('200px')
    unmount()
    const small = mount(CARDS_BASE.replace('name: C', 'name: C\n    cardSize: small'))
    expect(gridVar(small.el, '--base-card-w')).toBe('220px')
    unmount()
    const large = mount(CARDS_BASE.replace('name: C', 'name: C\n    cardSize: large'))
    expect(gridVar(large.el, '--base-card-w')).toBe('340px')
  })
})
...
describe('covers', () => {
  ...
  it('no image key on the view → no cover element at all', () => {
    ...
    expect(el.querySelector('.base-card__cover')).toBeNull()
  })
  ...
  it('a #rrggbb value renders a colour block, an http(s) URL goes straight into src', async () => {
    ...
    await settle()
    const colour = q<HTMLElement>(cardOf(el, 'colour.md'), '.base-card__cover')
    expect(colour.tagName).toBe('DIV')
    expect(colour.style.background).toMatch(/#ff0000|rgb\(255,\s*0,\s*0\)/i)
    const remote = q<HTMLImageElement>(cardOf(el, 'remote.md'), 'img.base-card__cover')
  ...
  it('a wikilink resolves through api.readAsset into a data: URL; a placeholder shows until it lands', async () => {
    ...
    expect(cardOf(el, 'local.md').querySelector('.base-card__cover--empty')).not.toBeNull()
    await settle()
    const img = q<HTMLImageElement>(cardOf(el, 'local.md'), 'img.base-card__cover')
  ...
  it('no value and a failed resolution both render the neutral placeholder', async () => {
    const { el } = mount(WITH_IMAGE, { records: COVER_RECORDS })
    await settle()
    expect(cardOf(el, 'bare.md').querySelector('.base-card__cover--empty')).not.toBeNull()
    expect(cardOf(el, 'broken.md').querySelector('.base-card__cover--empty')).not.toBeNull()
  ...
  it('one bridge call per unique ref: a grid of repeated covers is served from the cache', async () => {
    ...
    expect([...el.querySelectorAll('img.base-card__cover')]).toHaveLength(2)
  })

  it('imageFit and imageAspectRatio land as CSS custom properties; the defaults are cover and 1', async () => {
    const { el } = mount(WITH_IMAGE, { records: COVER_RECORDS })
    await settle()
    expect(gridVar(el, '--base-card-fit')).toBe('cover')
    expect(gridVar(el, '--base-card-ratio')).toBe('1')
    unmount()
    const tuned = mount(WITH_IMAGE.replace('image: cover', 'image: cover\n    imageFit: contain\n    imageAspectRatio: 1.5'), { records: COVER_RECORDS })
    await settle()
    expect(gridVar(tuned.el, '--base-card-fit')).toBe('contain')
    expect(gridVar(tuned.el, '--base-card-ratio')).toBe('1.5')
  })
})
