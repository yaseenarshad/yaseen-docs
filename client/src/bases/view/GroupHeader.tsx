...
export function summaryKindOf(view: BaseView, key: string): string | undefined {
...
export function GroupHeader({ def, view, columns, groupKey, rows, collapsed, onToggle }: GroupHeaderProps) {
  ...
  return (
    <div className="base-group">
      <button type="button" className="base-group__toggle" aria-expanded={!collapsed} aria-label={`Toggle group ${label}`} onClick={onToggle}>
      ...
      <span className={`base-group__value${groupKey === null ? ' base-group__value--none' : ''}`}>
      ...
      {columns.map((key) => {
        ...
        return (
          <span key={key} className="base-group__summary" title={`${propertyLabel(def, key)} ${kind}`}>
            <span className="base-group__summary-kind">{kind}</span>
