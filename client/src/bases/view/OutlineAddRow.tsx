  ...
  return (
    <li className="base-outline__add">
      ...
      {open && (matches.length > 0 || offerCreate) && (
        ...
        <div className="base-outline__picker" role="listbox" aria-label="Link a page suggestions" onMouseDown={(e) => e.preventDefault()}>
          {matches.map((c) => (
            <button key={c.path} type="button" role="option" aria-selected={false} className="base-outline__pick" onClick={() => pick(c.path)}>
              {c.folderPage && <BaseGlyph className="base-outline__glyph" />}
              {c.name}
            </button>
          ))}
          {offerCreate && (
            <button type="button" className="base-outline__pick base-outline__pick--create" onClick={create}>
              + Create '{typed}' here
            </button>
          )}
        </div>
