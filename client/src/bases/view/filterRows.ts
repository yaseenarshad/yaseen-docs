...
import type { FilterNode } from '../baseFile'
...
export type PropertyType = 'text' | 'number' | 'date' | 'checkbox' | 'list' | 'tags' | 'file'
...
export const OPERATORS_BY_TYPE: Record<PropertyType, OperatorId[]> = {
  ...
  date: ['dateIs', 'dateBefore', 'dateAfter', 'isEmpty'],
  checkbox: ['checked', 'unchecked'],
  list: ['contains', 'notContains', 'isEmpty'],
  tags: ['contains', 'notContains', 'isEmpty'],
...
/**
 * Property type from the first non-empty value across `records` (file fields by name, formulas
 * and unknowns default to text). A list is `tags` only for the `tags` property.
 */
export function inferType(property: string, records: readonly IndexRecord[]): PropertyType {
