...
const openTable = (el: ParentNode): void => click(q(el, '.base-tab__btn:nth-of-type(1)'))
/** Switch to the view named `name` (tabs are switch-only here). */
function selectView(el: ParentNode, name: string): void {
  const tab = [...el.querySelectorAll<HTMLElement>('.base-tab__btn')].find((b) => b.textContent === name)
...
describe('config edits are ONE settings write on the folder page', () => {
  it('a sort change goes back through the one door, whole-key, views verbatim', async () => {
    ...
    click([...el.querySelectorAll<HTMLElement>('.base-menu__action')].find((b) => b.textContent === 'Add sort')!)
    await flush()
