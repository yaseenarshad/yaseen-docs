...
export function useMenuEvents({ onOpenFolder, onOpenRoot, onCloseTab, onNextTab, onPrevTab }: UseMenuEventsOptions): void {
  useEffect(() => {
    const menu = window.yaseenDocs.menu
    const offs = [menu.onOpenFolder(onOpenFolder), menu.onOpenRoot(onOpenRoot), menu.onCloseTab(onCloseTab), menu.onNextTab(onNextTab), menu.onPrevTab(onPrevTab)]
    return () => offs.forEach((off) => off())
  }, [onOpenFolder, onOpenRoot, onCloseTab, onNextTab, onPrevTab])
}
