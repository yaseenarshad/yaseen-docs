...
export function ColumnSearch({ value, onChange, label, inputRef, ...props }: ColumnSearchProps) {
  const localRef = useRef<HTMLInputElement>(null)
  const ref = inputRef ?? localRef
  return (
    <div className="column-search">
      <SearchIcon />
      <input {...props} ref={ref} type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder="Search columns…" aria-label={label} autoComplete="off" />
      {value && (
