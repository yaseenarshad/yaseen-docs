...
import { setDisplayName } from './columnLabel'
...
export interface TableHeaderMenuProps {
  ...
  /** Every key the folder page already offers, declared columns included — "Add column" refuses a repeat. */
  taken: readonly string[]
  /** The folder page's own declarations, spread under the new one. */
  columns: Record<string, ColumnDecl>
  onUpdate: Mutate
  /** `FolderPageMode.setColumns`: declare on the folder page AND show, one write. Absent → no "Add column to the right…". */
  declareColumn?: (columns: Record<string, ColumnDecl>, views: ViewDef[]) => void
  /** "Delete column…" (YAZ-1513): hands the key to the table, which asks first (`ConfirmDeleteColumn`). Absent → not offered. */
 ...
 * prefilled with the current label — Enter commits, an emptied field goes back to the default,
 ...
 */
export function TableHeaderMenu({ x, y, columnKey, def, view, viewIndex, keys, taken, columns, onUpdate, declareColumn, onDeleteColumn, onClose }: TableHeaderMenuProps) {
        ...
        {mode === 'rename' ? (
          ...
          <AddColumn
            ...
            onSave={(name, column) => {
              declareColumn?.(
                { ...columns, [name]: column },
                def.views.map((v, i) => (i === viewIndex ? { ...v, order: insertAfter(keys, key, `note.${name}`) } : v)),
              )
              onClose()
            }}
          />
