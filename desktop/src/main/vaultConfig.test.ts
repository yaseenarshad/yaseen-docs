...
describe('readConfig', () => {
  it('missing folder or file → null, and reading NEVER creates .yaseendocs', async () => {
    ...
    expect(await readConfig(root, 'types.json')).toBeNull()
...
describe('writeConfig', () => {
  it('creates .yaseendocs lazily, writes pretty JSON + newline atomically, round-trips', async () => {
    const root = await makeRoot()
    await writeConfig(root, 'types.json', { version: 1, types: { due: 'date' } })
    const raw = await readFile(path.join(root, VAULT_CONFIG_DIR, 'types.json'), 'utf8')
    expect(raw).toBe(`${JSON.stringify({ version: 1, types: { due: 'date' } }, null, 2)}\n`)
    expect(await readConfig(root, 'types.json')).toEqual({ version: 1, types: { due: 'date' } })
...
describe('subscribeConfig', () => {
  it('a subscribe before the folder exists creates nothing and still sees an external folder + file creation', async () => {
    ...
    await mkdir(path.join(root, VAULT_CONFIG_DIR))
    await writeFile(path.join(root, VAULT_CONFIG_DIR, 'types.json'), '{"kind":"external"}')
    await until(() => changes.length >= 1)
    expect(changes[0]).toEqual({ root, name: 'types.json' })
  })
  ...
  it('an own write notifies synchronously, once — the watcher echo is deduped by mtime', async () => {
    ...
    const changes = collect(root)
    await sleep(200)
    await writeConfig(root, 'types.json', { a: 1 })
    expect(changes).toEqual([{ root, name: 'types.json' }]) // synchronous, before any watcher round-trip
  ...
  it('an own write straight after subscribing (folder created during watcher init) still leaves external edits watched', async () => {
    ...
    const root = await makeRoot()
    const changes = collect(root)
    await writeConfig(root, 'types.json', { a: 1 }) // no wait: races the watcher's init on purpose
    expect(changes).toEqual([{ root, name: 'types.json' }])
    await sleep(300)
    await writeFile(path.join(root, VAULT_CONFIG_DIR, 'types.json'), '{"a":2}')
    await until(() => changes.length >= 2)
    expect(changes[1]).toEqual({ root, name: 'types.json' })
  })
  ...
  it('external change and unlink notify {root, name}; non-json files never do', async () => {
    ...
    await until(() => changes.length >= 1)
    expect(changes[0]).toEqual({ root, name: 'types.json' })
    await rm(path.join(root, VAULT_CONFIG_DIR, 'types.json'))
    await until(() => changes.length >= 2)
    expect(changes[1]).toEqual({ root, name: 'types.json' })
