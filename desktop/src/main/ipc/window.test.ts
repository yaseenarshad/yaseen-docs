...
const RIGHT = { open: true, width: 520, items: [abs('/v/right.md')], expanded: abs('/v/right.md') }
const entry: WindowEntry = { id: 'w1', root: abs('/v'), file: abs('/v/a.md'), tabs: [abs('/v/a.md')], rightPanel: RIGHT, sidebarCollapsed: false, bounds }
...
describe('registerWindowIpc', () => {
  ...
  it('window:identity answers the complete per-window identity for a registered sender', async () => {
    expect(await registered(CH.windowIdentity)({ sender })).toEqual(ok({ id: 'w1', root: abs('/v'), file: abs('/v/a.md'), tabs: [abs('/v/a.md')], rightPanel: RIGHT, sidebarCollapsed: false }))
  })
  ...
  it('window:set-identity merges root / file into the entry, keeping id and bounds; tabs follow the invariant', async () => {
    ...
    expect(await registered(CH.windowSetIdentity)({ sender }, { root: abs('/other'), file: null })).toEqual(ok(undefined))
    expect(store.get().windows).toEqual([{ id: 'w1', root: abs('/other'), file: null, tabs: [], rightPanel: RIGHT, sidebarCollapsed: false, bounds }])
    expect(await registered(CH.windowSetIdentity)({ sender }, { file: abs('/other/b.md') })).toEqual(ok(undefined))
    expect(store.get().windows).toEqual([{ id: 'w1', root: abs('/other'), file: abs('/other/b.md'), tabs: [abs('/other/b.md')], rightPanel: RIGHT, sidebarCollapsed: false, bounds }])
  ...
  it('window:set-identity accepts a tabs patch: de-duplicated, and the active file is prepended when missing (GRO-2232)', async () => {
    expect(await registered(CH.windowSetIdentity)({ sender }, { tabs: [abs('/v/a.md'), abs('/v/b.md'), abs('/v/a.md')] })).toEqual(ok(undefined))
    expect(store.get().windows[0].tabs).toEqual([abs('/v/a.md'), abs('/v/b.md')])
    // The invariant holds on the entry AS WRITTEN: a tabs patch missing the untouched active file repairs by prepending.
    expect(await registered(CH.windowSetIdentity)({ sender }, { tabs: [abs('/v/b.md'), abs('/v/c.md')] })).toEqual(ok(undefined))
    expect(store.get().windows[0].tabs).toEqual([abs('/v/a.md'), abs('/v/b.md'), abs('/v/c.md')])
    // file and tabs patched together: the new file leads.
    expect(await registered(CH.windowSetIdentity)({ sender }, { file: abs('/v/b.md'), tabs: [abs('/v/b.md'), abs('/v/c.md')] })).toEqual(ok(undefined))
    expect(store.get().windows[0]).toEqual({ id: 'w1', root: abs('/v'), file: abs('/v/b.md'), tabs: [abs('/v/b.md'), abs('/v/c.md')], rightPanel: RIGHT, sidebarCollapsed: false, bounds })
  })

  it('window:set-identity accepts one complete right-panel patch and enforces main/right exclusivity', async () => {
    const rightPanel = { open: true, width: 600, items: [abs('/v/a.md'), abs('/v/b.md'), abs('/v/b.md')], expanded: abs('/v/a.md') }
    expect(await registered(CH.windowSetIdentity)({ sender }, { rightPanel })).toEqual(ok(undefined))
    expect(store.get().windows[0].rightPanel).toEqual({ open: true, width: 600, items: [abs('/v/b.md')], expanded: null })
  })
  ...
  it('window:set-identity rejects the whole call on any bad tabs element, leaving the entry untouched', async () => {
    ...
    expect(await registered(CH.windowSetIdentity)({ sender }, { tabs: [abs('/v/a.md'), 'rel.md'] })).toEqual(bad('NOT_ABSOLUTE'))
    expect(await registered(CH.windowSetIdentity)({ sender }, { tabs: [abs('/v/a.md'), 5] })).toEqual(bad('NOT_ABSOLUTE'))
  ...
  it('window:set-identity validates the patch', async () => {
    ...
    expect(await registered(CH.windowSetIdentity)({ sender: stranger }, { root: abs('/v') })).toEqual(bad('BAD_REQUEST'))
    store.removeWindow('w1')
    expect(await registered(CH.windowSetIdentity)({ sender }, { root: abs('/v') })).toEqual(bad('NOT_FOUND'))
  })

  it('window:open validates the options and hands them to the manager (absent paths read as null)', async () => {
    expect(await registered(CH.windowOpen)({ sender }, { root: abs('/v'), file: abs('/v/a.md') })).toEqual(ok(undefined))
    expect(manager.openWindow).toHaveBeenCalledWith({ root: abs('/v'), file: abs('/v/a.md') })
