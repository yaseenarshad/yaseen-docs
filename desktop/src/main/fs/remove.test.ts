...
describe('removeEntry (GRO-2272 A1)', () => {
  ...
  it('refuses dot-entries — invisible infrastructure is never deletable from a UI that never showed it', async () => {
    trashResolves()
    for (const p of [path.join(root, '.obsidian'), path.join(root, '.yaseendocs'), path.join(root, '.hidden.md')]) {
      const err = await failure(removeEntry({ path: p }))
