 ...
 * and the table's header row follows. Same harness as its siblings (temp `--user-data-dir`,
 * COPY of the fixture, step screenshots); serial by design.
 */
...
test('step 1 — every shown row carries a grip, and the arrows are gone', async () => {
  ...
  await expect(contents().locator('[aria-label="Reorder file.name"]')).toBeVisible()
...
test("step 2 — dragging unit's grip onto kpi_category's top half reorders the durable order and the table follows", async () => {
  await contents().locator('[aria-label="Reorder unit"]').dragTo(rowOf('kpi_category'), {
...
test('step 3 — file.name itself can be dragged down, like the arrows always allowed', async () => {
  ...
  await contents().locator('[aria-label="Reorder file.name"]').dragTo(rowOf('kpi_category'), {
    targetPosition: { x: 10, y: 2 },
  })
  await expect(headers()).toHaveText(['unit', 'file.name', 'kpi_category', 'funnel_stages'])
  await expect
    .poll(async () => {
      const text = await readFile(path.join(vault, 'KPIs.md'), 'utf8')
