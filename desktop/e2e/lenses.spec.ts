 ...
 *
 * Topics is the DEFAULT lens and, until YAZ-848 fills it, an empty shell; Files is today's file
 * explorer, unchanged, behind a tab. The choice is GLOBAL app state (`AppState.sidebarLens`), so
 * it must survive quit → relaunch the way `sidebarWidth` does (easyWave step 2's shape).
 *
...
test('step 1 — a state file with no lens boots on TOPICS: the placeholder body, no tree', async () => {
  ...
  await expect(bodyMsg(win)).toContainText('YAZ-848') // the shell 6B replaces
...
test('step 3 — a query replaces the ACTIVE tab\'s body; the tabs row stays, and switching keeps the query', async () => {
  ...
  await expect(treeRows(win)).toHaveCount(0) // the flat list replaces the FILES body it was typed on
  ...
  await expect(bodyMsg(win)).toContainText('YAZ-848')
  await expect(treeRows(win)).toHaveCount(0)
})
